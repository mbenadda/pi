...
Upstream 0.85.1 makes the experimental client/server commands and the `client`/`experimental/plugin` npm exports development-only: the published CLI no longer dispatches `pi server`/`pi client`, and those packages resolve only from source. This fork keeps that boundary intact and adds an explicit, unpublished standalone runtime beside it:

- `src/main.ts`, `src/cli.ts`, `src/bun/cli.ts`, and `tsconfig.build.json` stay upstream-clean: the published `dist/index.js`, `dist/cli.js`, and the bundled `dist/bundle/cli.js` never import the development-only trees, and `package.json` keeps the source-only `./client` and `./experimental/plugin` exports plus the `files` allowlist that excludes `dist/client`, `dist/experimental`, and `dist/cli/experimental` from the tarball. The stable `src/bun/cli.ts` stays upstream-clean and keeps its published-entry role, but it is no longer the binary build entry: `build:binary` and `scripts/build-binaries.sh` compile `src/experimental/bun-cli.ts` instead.
- `src/experimental/standalone-cli.ts` is the fork's standalone entrypoint: it dispatches internal process roles (coordinator, server, session-worker), the experimental `server`, `client`, and `workspace` commands behind `PI_EXPERIMENTAL=1`, `piw`, and then falls back to the stable `main()`. `src/experimental/bun-cli.ts` is the compiled-Bun counterpart and the sole entrypoint compiled into the standalone binaries (`build:binary` and `scripts/build-binaries.sh`). Unlike the npm package, the GitHub standalone binaries built from it therefore embed the experimental dispatch: internal roles, `piw`, and the `server`/`client`/`workspace` commands behind `PI_EXPERIMENTAL=1` ship inside the executable, while the npm tarball never contains the development-only trees at all.
- `tsconfig.standalone.json` compiles the development-only trees (with the stable trees they import) into `dist`; `npm run build` runs it after the upstream build. The output is unpublished (see the `files` allowlist).
- The checkout-backed development path runs the server through `node <checkout>/packages/coding-agent/dist/experimental/standalone-cli.js server`; the pinned backend runs `pi-workspace-server server` from the standalone binary. Both share `src/experimental/commands.ts` for dispatch with the development entrypoint `src/experimental/cli.ts`.
- `scripts/check-runtime-deps.mjs` validates the standalone build too: exactly the development-only trees (`src/client`, `src/experimental`, `src/cli/experimental`) may import `@earendil-works/pi-client`, `pi-protocol`, and `pi-server`, and nothing else may. `scripts/check-entry-graphs.mjs` fails if any published entry (`src/index.ts`, `src/cli.ts`, `src/rpc-entry.ts`) reaches the standalone-only trees, `test/package-distribution.test.ts` and `test/experimental-cli-entry.test.ts` lock these decisions in, and `npm run check:package-install` packs and installs the tarball to prove the published package works without the development-only code.

### Included stable changes

The 0.85.1 TUI improvements are inherited by the Workspace client because it renders through the same `createInteractiveTui`/`createChatViewport`/tool-renderer components: alt-mode scrollbar and accelerated Alt-wheel scrolling, the clickable jump-to-end indicator, linearly scaling fullscreen transcript search, mouse-hover-stable list selection, the shared working-status indicator restyle (the editor-border embedded indicator is a stable-editor change the experimental client does not use; it keeps its standalone indicator), Zed terminal capability detection, and the SIGWINCH seccomp fix. Also included: statically linked musl `fd`/`ripgrep` downloads on Linux with GitHub-API-free version resolution, skills availability with bash-only tools, isolated concurrent session shares, non-EXIF APP1 segment scanning, in-memory session forks and external entry ingestion, GPT-6 Astra and refreshed model catalogs, NO_PROXY subdomain matching, and the theme-marker and footer fixes.
...
- Local session utilities tied to the stable session manager (HTML export, branch summaries, in-memory fork UX) have no Workspace equivalent: they are not exposed to the client, and no remote slash commands exist for them. The only remote slash commands are `/model`, `/thinking`, `/compact`, and `/reload`.

## Source development path

`PI_EXPERIMENTAL=1 node packages/coding-agent/dist/experimental/standalone-cli.js workspace --ssh-host <host> --remote-cwd <path>` (after `npm run build`, or the `src/experimental/standalone-cli.ts` source entry with the source resolver) remains a checkout-backed development command; the published `pi` entrypoint no longer dispatches it. It retains the older exact-revision staging path for protocol development. Installed `piw` never enters that path: it discovers its embedded manifest next to the executable and fails closed if the manifest, checksum, platform, protocol, or backend archive is missing or inconsistent.
